...
bool block_t::validate()
{
    ...
    // valid = validate_block_helper(transactions, balances, prev_hash, reward_addr, blk_hash, BLOCK_REWARD);
    


    bool tx_valid = true;
    for (auto tx : transactions) {
        tx_valid = tx_valid && tx->validate();
    }
    if (!tx_valid)
    {
        valid = false;
    ...
    for (auto tx : transactions) {
        bool bal_avail = tx->balance_available(balances);
        if(!bal_avail)
        {
            valid = false;
            return false;
    ...
    hash_result_t block_hash;
    compute_block_hash(block_hash);

    bool block_hash_valid = true;
    for(unsigned i=0; i != SHA256_DIGEST_LENGTH; i++) {
        block_hash_valid = block_hash_valid && (block_hash[i] == blk_hash[i]);
        if(!block_hash_valid) 
        {
            valid = false;
            return false;
