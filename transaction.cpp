...
bool txn_t::validate() 
{
    ...
    hash_result_t transaction_hash;
    ...
    SHA256_Update(&sha256, change_addr.data(), change_addr.size());
    SHA256_Update(&sha256, &amount, sizeof(amount));
    SHA256_Final(const_cast<uint8_t*>(transaction_hash.data()), &sha256);

    for(unsigned i=0; i != SHA256_DIGEST_LENGTH; i++) {
        heq = heq && (transaction_hash[i] == tx_hash[i]);
        if(!heq) 
        {
            valid = false;
            return false;
